...
int main()
{
	...
	for (int j = 0; j < tokennum; j++)
	{
		...
		int hashingnum = PreHashing(token[j]);
	...
	for (int j = 0; j < tokennum; j++)
	{
		...
		//設定找到token時印出訊息
		int hashingnum = PreHashing(token_test1[j]);
	...
	for (int j = 0; j < tokennum; j++)
	{
		...
		//設定找到token時印出訊息
		int hashingnum = PreHashing(token_test2[j]);
		string find = hash.Search(hashingnum);
	...
	for (int j = 0; j < tokennum; j++)
	{
		...
		//設定找到token時印出訊息
		int hashingnum = PreHashing(token_test3[j]);
		string find = hash.Search(hashingnum);
	...
	do
	{
		...
		if (selection == 1)     //插入key值
		{
			...
			int hashingnum = PreHashing(user_token);
			string find = hash.Search(hashingnum);			// 先查詢此key值是否已存在BST中
